	...
	t.Run("allocate meta pages within a single page", func(t *testing.T) {
		...
		for i := 0; i < nm; i++ {
			...
			if offset/int64(pf.pageSize) != 0 {
				t.Fatalf("expected all meta pages to be in page 0, got page: %v", offset/int64(pf.pageSize))
			}
		}
	})
	...
	t.Run("reuse freed meta page", func(t *testing.T) {
		...
		for i := 0; i < n; i++ {
			...
			if i == 0 {
				...
				expectedOffset := lastOffset + int64(pf.metaSize)
