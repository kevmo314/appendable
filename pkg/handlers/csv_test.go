...
func TestCSV(t *testing.T) {
	...
	t.Run("correctly sets field offset", func(t *testing.T) {
		...
		rv1, mp1, err := collected[0].BTree(&btree.BTree{Data: r2, DataParser: CSVHandler{}}).Find(btree.ReferencedValue{Value: []byte("test1")})
		if err != nil {
			t.Fatal(err)
		}
		...
		rv2, mp2, err := collected[0].BTree(&btree.BTree{Data: r2, DataParser: CSVHandler{}}).Find(btree.ReferencedValue{Value: []byte("test2")})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("existing index but different type", func(t *testing.T) {
		...
		rv2, mp2, err := collected[1].BTree(&btree.BTree{Data: r2, DataParser: CSVHandler{}}).Find(btree.ReferencedValue{Value: v2})
		if err != nil {
			t.Fatal(err)
		}
		if mp2 == (pointer.MemoryPointer{}) {
	...
	t.Run("correctly iterates through btree", func(t *testing.T) {
		...
		iter, err := collected[0].BTree(&btree.BTree{Data: r2, DataParser: JSONLHandler{}}).Iter(btree.ReferencedValue{Value: v2})
		if err != nil {
			t.Fatal(err)
		}
