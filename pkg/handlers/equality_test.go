...
func compareMetaPages(i1, i2 []*btree.LinkedMetaPage, jr, cr []byte) (bool, string) {
	...
	for i, collected1 := range i1 {
		...
		if i == 0 {
			...
			for _, val := range h1 {
				_, found, err := collected1.BPTree(jr).FindFirst([]byte(val))
				...
				_, found, err = collected2.BPTree(cr).FindFirst([]byte(val))
			...
			for _, val := range h2 {
				...
				_, found, err := collected1.BPTree(jr).FindFirst(v2)
				...
				_, found, err = collected2.BPTree(cr).FindFirst(v2)
